    ...
    WebGLViewer.rt_shader    = {
      ...
      fragment_source: `precision highp int;
                        ...
                        void main()
                        {
                            ...
                            vec2 pixel          = gl_FragCoord.xy - offset;
...
function rt_init(session)
{
    ...
    let glLayer = new XRWebGLLayer(session, gl, { ignoreDepthValues: true });
...
function rt_draw(session, pose) 
{
    ...
    for (let view of pose.views) {
      ...
      // Pixel Offset
      index = gl.getUniformLocation(WebGLViewer.rt_shader.program, "offset");
      gl.uniform2fv(index, [viewport.x, viewport.y]);
      ...
      glMatrix.mat4.mul(viewInvMatrix, view.transform.inverse.matrix, WebGLViewer.camera.view);
      glMatrix.mat4.invert(viewInvMatrix, viewInvMatrix);
